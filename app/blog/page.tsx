...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Dacă ești tu, ai generări nelimitate (999)
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Dacă ești tu, setează generări nelimitate
  ...
  const exportToPDF = async () => {
    ...
    html2pdf(element, {
      margin: 0.5,
      filename: `articolai-${topic || 'articol'}.pdf`,
  ...
  const shareOnTwitter = () => {
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(`Am generat un articol cu ArticolAI: ${topic}`)}&url=${encodeURIComponent(window.location.href)}`, '_blank');
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Dacă ești tu, nu scădea generările
  ...
  return (
    <div style={{ fontFamily: 'Inter, system-ui, sans-serif', minHeight: '100vh', backgroundColor: '#f8f8f8', color: '#1a1a1a' }}>
      ...
      <nav style={{ backgroundColor: '#fff', borderBottom: '1px solid #ebebeb', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            {!isLoggedIn ? (
              <button
                ...
                style={{ background: '#7C3AED', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 18px', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
              >
                Intră în cont
              </button>
            ) : (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                {hasSubscription ? (
                  <span style={{ fontSize: 12, background: '#EDE9FE', color: '#6D28D9', padding: '4px 12px', borderRadius: 20, fontWeight: 600 }}>
                    Premium
                  </span>
                ) : (
                  <span style={{ fontSize: 12, background: '#FEF3C7', color: '#92400E', padding: '4px 12px', borderRadius: 20, fontWeight: 500 }}>
                    {generationsLeft} generări rămase
                  </span>
                )}
                <div style={{ position: 'relative' }}>
                  <button
                    onClick={() => setShowUserMenu(!showUserMenu)}
                    style={{ background: '#f3f4f6', border: 'none', borderRadius: 8, padding: '7px 14px', fontSize: 13, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}
                  >
                    Contul meu ▾
                  </button>
                  {showUserMenu && (
                    <div style={{ position: 'absolute', right: 0, top: 40, background: '#fff', border: '1px solid #ebebeb', borderRadius: 10, padding: 8, minWidth: 200, boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
                      {!hasSubscription && (
                        <button
                          onClick={handleSubscribe}
                          style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 14px', fontSize: 13, fontWeight: 600, color: '#7C3AED', background: '#EDE9FE', border: 'none', borderRadius: 8, cursor: 'pointer', marginBottom: 4 }}
                        >
                          Upgrade la Premium — 49.99 RON/lună
                        </button>
                      )}
                      <button
                        onClick={handleLogout}
                        style={{ display: 'block', width: '100%', textAlign: 'left', padding: '9px 14px', fontSize: 13, color: '#666', background: 'none', border: 'none', borderRadius: 8, cursor: 'pointer' }}
                      >
                        Deconectare
                      </button>
                    </div>
